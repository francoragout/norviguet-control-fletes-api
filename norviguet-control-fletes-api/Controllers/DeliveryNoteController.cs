...
namespace norviguet_control_fletes_api.Controllers
{
    ...
    public class DeliveryNoteController : ControllerBase
    {
        ...
        public DeliveryNoteController(ApplicationDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public async Task<IActionResult> UpdateDeliveryNote(int id, [FromBody] UpdateDeliveryNoteDto dto)
        {
            ...
            if (deliveryNote.Order.Status != OrderStatus.Pending)
            {
                return Conflict(new
                {
                    ...
                    message = "Cannot edit delivery note linked to a closed or rejected order."
                });
            }
            ...
            if (deliveryNote.Status != DeliveryNoteStatus.Pending)
            {
                return Conflict(new
                {
                    ...
                    message = "Cannot edit a delivery note that is approved or cancelled."
                });
            }
        ...
        public async Task<IActionResult> DeleteDeliveryNote(int id)
        {
            ...
            if (deliveryNote.Order.Status != OrderStatus.Pending)
            {
                ...
                {
                    code = "ORDER_CLOSED_OR_REJECTED",
                    message = "Cannot delete delivery note linked to a closed or rejected order."
                });
            }
            ...
            if (deliveryNote.Status != DeliveryNoteStatus.Pending)
            {
                ...
                {
                    code = "APPROVED_OR_CANCELLED_DELIVERY_NOTE",
                    message = "Cannot delete a delivery note that is approved or cancelled."
                });
            }
        ...
        public async Task<IActionResult> DeleteDeliveryNotesBulk([FromBody] DeleteEntitiesDto dto)
        {
            ...
            if (notesToDelete.Count < deliveryNotes.Count)
            {
                return Conflict(new
                {
                    code = "APPROVED_OR_CANCELLED_DELIVERY_NOTE_CLOSED_OR_REJECTED_ORDER",
